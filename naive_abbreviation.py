...
def fread(fname, encoding=encoding):
    f = open(os.path.abspath(os.path.expanduser(fname)),encoding=encoding)
    try:
        for l in f:
            line = l.strip()
            if line:
...
class Candidate(str):
    ...
    def __getitem__(self, item):
        if isinstance(item, int):
            ...
            stop = self.start + item
            s = str.__getitem__(self, item)
            return Candidate(start, stop, s)
        else:
            start = self.start + max(0,item.start)
            stop = self.start + min(item.stop,len(self))
            s = str.__getitem__(self, item)
            return Candidate(start, stop, s)
...
def main(fname, verbose=False, encoding=encoding):
            ...
            try:
                for candidate in getcandidates(sentence):
                    try:
                        ...
                        if verbose:
                            print(i, 'Omitting candidate', candidate.encode(encoding), file=sys.stderr)
                            print('Reason:', e.args[0].encode(encoding), '\n', file=sys.stderr)
                        omit += 1
                    else:
                        try:
                            ...
                            if verbose:
                                # print >>sys.stderr, i, sentence
                                print(i, 'Omitting candidate', definition.encode(
                            ...
                            if verbose:
                                # print >>sys.stderr, i, sentence
                                print(i, 'Omitting candidate', definition.encode(
                                    encoding), '||', candidate.encode(encoding), file=sys.stderr)
                                print('Reason:', e.args[0].encode(encoding), '\n', file=sys.stderr)
                            omit += 1
                        else:
    ...
    print('INFO: %d abbreviations detected and kept (%d omitted)' % (written, omit), file=sys.stderr)
...
def score(gold, pred):
    ...
    fp = 0;
    fn = 0
    for id, gvalues in list(golds.items()):
        for pair in gvalues:
            try:
                pvalues = preds[id]
            except KeyError:
                ...
                if pair in pvalues:
                    ...
                    print()
                    'false negative', id, pair
                    fn += 1

    for id, pvalues in list(preds.items()):
        fp += len(pvalues)

    # The number of gold abbreviatios
    total = 0
    for v in list(golds.values()):
        total += len(v)
...
if __name__ == '__main__':
    ...
    # main('test.txt', verbose=True)


    main('D:/allenai-paper-corpus/part-00000', verbose=False)
